...
public:
  ...
  static inline float GetValue(const Vector3 &p) {
    ...
    // Interpolate results
    const float l1 = Math::Lerp(Gradient(Perm[aa], x, y, z),
                                Gradient(Perm[ba], x - 1, y, z), u);
    const float l2 = Math::Lerp(Gradient(Perm[ab], x, y - 1, z),
                                Gradient(Perm[bb], x - 1, y - 1, z), u);
    const float l3 = Math::Lerp(Gradient(Perm[aa + 1], x, y, z - 1),
                                Gradient(Perm[ba + 1], x - 1, y, z - 1), u);
