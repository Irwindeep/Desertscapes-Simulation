...
*/
inline Box2D::Box2D() : a(Vector2(0)), b(Vector2(0)) {}
...
inline Box2D::Box2D(const Box2D &b1, const Box2D &b2)
    : a(Vector2::Min(b1.a, b2.a)), b(Vector2::Max(b1.b, b2.b)) {}
...
*/
inline Vector2 Box2D::Center() const { return (a + b) / 2; }
...
*/
inline Vector2 Box2D::Size() const { return b - a; }
...
// ScalarField2D. Represents a 2D field (nx * ny) of scalar values bounded in
// world space. Can represent a heightfield.
class ScalarField2D {
protected:
  Box2D box;
  int nx, ny;
...
public:
  ...
  */
  ...
  /*
  \brief Compute a vertex world position in 3D, with the scalar value treated as
  height.
  */
  inline Vector3 Vertex(int i, int j) const {
    float x =
        box.Vertex(0).x + i * (box.Vertex(1).x - box.Vertex(0).x) / (nx - 1);
    float y = Get(i, j);
    float z =
        box.Vertex(0).y + j * (box.Vertex(1).y - box.Vertex(0).y) / (ny - 1);
  ...
  */
  inline Vector3 Vertex(const Vector2i &v) const {
    float x =
        box.Vertex(0).x + v.x * (box.Vertex(1).x - box.Vertex(0).x) / (nx - 1);
    float y = Get(v.x, v.y);
    float z =
        box.Vertex(0).y + v.y * (box.Vertex(1).y - box.Vertex(0).y) / (ny - 1);
    return Vector3(z, y, x);
  }
  ...
  */
  ...
  \brief Utility.
  */
  inline int ToIndex1D(const Vector2i &v) const { return v.x * nx + v.y; }
  ...
  */
  inline Vector2 BottomLeft() const { return box.Vertex(0); }

  /*!
  \brief Returns the top right corner of the bounding box.
  */
  inline Vector2 TopRight() const { return box.Vertex(1); }
