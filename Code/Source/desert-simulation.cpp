...
static Vector2i next8[8] = {Vector2i(1, 0),  Vector2i(1, 1),  Vector2i(0, 1),
                            Vector2i(-1, 1), Vector2i(-1, 0), Vector2i(-1, -1),
                            Vector2i(0, -1), Vector2i(1, -1)};
static Vector2i Next(int i, int j, int k) { return Vector2i(i, j) + next8[k]; }
...
#pragma omp atomic
  ...
  // (3) Jump downwind by saltation hop length (wind direction). Repeat until
  // sand is deposited.
  int destI = startI;
  int destJ = startJ;
  ...
  while (bounce < MAX_BOUNCE) {
    ...
    // Abrasion of the bedrock occurs with low sand supply, weak bedrock and a
    // low probability.
...
#pragma omp atomic
    ...
    else if (sediments.Get(destID) > 0.0 &&
             p < 0.6 + (vegetationOn ? (vegetation.Get(destID) * 0.4) : 0.0)) {
#pragma omp atomic
      sediments[destID] += matterToMove;
      break;
    }
    // Empty cell - 40% chance of deposition (if vegetation == 0.0)
             ...
             p < 0.4 + (vegetationOn ? (vegetation.Get(destID) * 0.6) : 0.0)) {
#pragma omp atomic
      sediments[destID] += matterToMove;
      break;
    }
  ...
  // End of the deposition loop - we have move matter from (startI, startJ) to
  // (destI, destJ)
...
of reptation, we didn't find any particular change with or without reptation
activated. Still, implementation is provided if someone wants to try it.
*/
void DuneSediment::PerformReptationOnCell(int i, int j, int bounce) {
  ...
  float nslope[8];
  int n =
      Math::Min(2, CheckSedimentFlowRelative(
                       Vector2i(i, j), tanThresholdAngleSediment, nei, nslope));
  int nEffective = 0;
  for (int k = 0; k < n; k++) {
    Vector2i next = nei[k];
    float sei = se / n;
...
#pragma omp atomic
    ...
    // Count the amount of neighbour which received sand from the current cell
    // (i, j)
    nEffective++;
  }
...
float DuneSediment::IsInShadow(int i, int j, const Vector2 &windDir) const {
  ...
  const Vector2 windStep = Vector2(windDir[0] > 0.0f   ? windStepLength
                                   : windDir[0] < 0.0f ? -windStepLength
                                                       : 0.0f,
                                   windDir[1] > 0.0f   ? windStepLength
                                   : windDir[1] < 0.0f ? -windStepLength
                                                       : 0.0f);
  Vector2 p = bedrock.ArrayVertex(i, j);
  Vector2 pShadow = p;
  ...
  while (true) {
    ...
    float t = (step / d);
    float s = Math::Step(t, tanThresholdAngleWindShadowMin,
                         tanThresholdAngleWindShadowMax);
    ret = Math::Max(ret, s);
  }
  return ret;
}
