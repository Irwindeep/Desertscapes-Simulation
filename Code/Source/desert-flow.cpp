...
static float length8[8] = {1.0f, sqrtf(2.0f), 1.0,  sqrtf(2.0f),
                           1.0f, sqrtf(2.0f), 1.0f, sqrtf(2.0f)};
static Vector2i Next(int i, int j, int k) { return Vector2i(i, j) + next8[k]; }

/*!
\brief Compute the flow directions at a given point. Returns an integer
representing the number of neighbour to distribute the material to. Arrays nei
and nslope contains respectively the neighbours in grid coordinates and the unit
slopes.
\param p Point.
...
*/
int DuneSediment::CheckSedimentFlowRelative(const Vector2i &p,
...
\param nslope returned unit slope array.
*/
int DuneSediment::CheckBedrockFlowRelative(const Vector2i &p,
                                           float tanThresholdAngle,
...
/*!
\brief Stabilize a given grid vertex with the use of CheckSedimentFlowRelative()
function. Used by multi-thread functions, but can also be used in a
...
/*!
\brief Stabilize a given grid vertex with the use of CheckBedrockFlowRelative()
function. Used by multi-thread functions, but can also be used in a
single-thread context.
\param i x coordinate
\param j y coordinate
